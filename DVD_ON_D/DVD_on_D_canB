...
byte txBuf0[] = {0x00, 0x05, 0x80, 0x20, 0x50, 0x00, 0x00}; // включен P, мотор заведён, тормоз отпущен.
byte txBuf1[] = {0x00, 0x05, 0x84, 0x21, 0x44, 0x00, 0x00}; // включен D, мотор заведён, тормоз нажат.
...
{
  Serial.begin(115200);
  // инициализация CAN1 bus, baudrate: CAN_83K3BPS - 83,3Kbit/s
  if(CAN0.begin(MCP_ANY, CAN_83K3BPS, MCP_8MHZ) == CAN_OK){
  Serial.print("CAN0: Init OK!\r\n");
  CAN0.setMode(MCP_NORMAL);
  } else Serial.print("CAN0: Init Fail!!!\r\n");
  
  // инициализация CAN1 bus, baudrate: CAN_83K3BPS - 83,3Kbit/s
  if(CAN1.begin(MCP_ANY, CAN_83K3BPS, MCP_8MHZ) == CAN_OK){
...
void loop(){  
  if(!digitalRead(2)){          // на втором пине INT от адаптера "в машину". Считываем, если он в LOW.
    ...
    if(rxId == 0x003){                // в сообщениях с этим ID передаются данные о положении ручки.
